...
import findErrors from '../dependencies/Errors/mongooseError/findError.js'
import saveErrors from '../dependencies/Errors/mongooseError/saveError.js'
import compareError from '../dependencies/Errors/bcryptErrors/compareError.js'
import signErrors from '../dependencies/Errors/jwtError/signError.js'
...
const getUserById = async (req, res) => {
    ...
    try{
      ...
      fetchedUser = await Users.findOne({ _id: user._id });
    }
    catch(err){
      findErrors(err,res)
    }
...
const getSavedCsv = async (req, res) => {
  try {
    ...
    try{
      ...
      fetchedFiles = await Sales.find({ uploadedBy: user._id });
    }
    catch(err){
      findErrors(err,res)
    }
...
const forgetPassword = async (req, res, next) => {
  ...
  try {
    ...
    try{
      ...
      user = await Users.findOne({ email: { $regex: email, $options: 'i' } });
    }
    catch(err){
      findErrors(err,res)
    }
